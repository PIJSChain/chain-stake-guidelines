...
RELEASE_EXCLUDE_REGEX='-mainnet$'    # 排除主网 tag
...
fetch_latest_version() {
    print_step "1" "查询最新测试网版本"

    local response
    response=$(download_text "$RELEASES_API")

    # 提取 tag_name 列表（GitHub 默认按时间倒序），跳过主网 tag 后取第一个
