...
# ============================================================
# Picks the most recent TestNet release (any tag NOT ending with `-mainnet`).
# PIJSChain/pijs hosts both networks: TestNet tags use no suffix or `-testnet`,
# MainNet tags always end with `-mainnet` — this script excludes the latter.
# ============================================================
...
$ReleaseExcludePattern = "*-mainnet"  # Skip MainNet tags
...
function Get-LatestVersion {
    Write-Step "2" "Checking the latest TestNet release"
    ...
    # GitHub returns releases newest first; pick the first one that is NOT a MainNet tag
    $testnetRelease = $releases | Where-Object { $_.tag_name -notlike $ReleaseExcludePattern } | Select-Object -First 1

    if (-not $testnetRelease) {
        Write-Error-Custom "No TestNet release found"
        exit 1
    }

    $script:LatestVersion = $testnetRelease.tag_name
    $script:ReleaseDownloadBase = "https://github.com/$GitHubRepo/releases/download/$($script:LatestVersion)"
    Write-Success "Latest TestNet version: $($script:LatestVersion)"
}
