...
RELEASE_EXCLUDE_REGEX='-mainnet$'    # Skip MainNet tags
...
fetch_latest_version() {
    print_step "1" "Checking the latest TestNet release"

    local response
    response=$(download_text "$RELEASES_API")

    # Extract tag_name list (newest first) and skip MainNet (-mainnet) tags
    LATEST_VERSION=$(printf '%s' "$response" \
        | grep -oE '"tag_name"[[:space:]]*:[[:space:]]*"[^"]+"' \
        | sed -E 's/.*"([^"]+)"$/\1/' \
        | grep -vE -- "$RELEASE_EXCLUDE_REGEX" \
        | head -1)

    if [ -z "$LATEST_VERSION" ]; then
        print_error "No TestNet release found"
        exit 1
    fi

    GITHUB_RELEASE="https://github.com/${GITHUB_REPO}/releases/download/${LATEST_VERSION}"
    print_success "Latest TestNet version: $LATEST_VERSION"
}
