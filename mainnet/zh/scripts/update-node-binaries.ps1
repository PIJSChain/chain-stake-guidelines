...
# 不修改链数据，不更新创世配置
# ============================================================
# 仅匹配 PIJSChain/pijs 中后缀为 `-mainnet` 的最新 release tag。
# 该仓库同时承载测试网（无后缀 / `-testnet`）与主网（`-mainnet`）release，
# 直接调用 `releases/latest` 可能误抓测试网最新版，因此主网脚本按 tag 后缀过滤。
# ============================================================
...
$ReleaseTagPattern = "*-mainnet"     # 仅匹配主网 release tag
...
function Get-LatestVersion {
    ...
    Write-Success "最新主网版本: $($script:LatestVersion)"
}
