...
RELEASE_TAG_REGEX='-mainnet$'        # 仅匹配主网 release tag
...
fetch_latest_version() {
    print_step "1" "查询最新主网版本"

    local response
    response=$(download_text "$RELEASES_API")

    # 提取 tag_name 列表（GitHub 默认按时间倒序），筛选主网 tag 后取第一个
    ...
    if [ -z "$LATEST_VERSION" ]; then
        print_error "未找到主网 release（要求 tag 以 -mainnet 结尾）"
        exit 1
    fi

    GITHUB_RELEASE="https://github.com/${GITHUB_REPO}/releases/download/${LATEST_VERSION}"
    print_success "最新主网版本: $LATEST_VERSION"
}
