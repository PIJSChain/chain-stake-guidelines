...
| 资源 | 下载地址 |
|------|---------|
| 节点程序 | https://github.com/PIJSChain/pijs/releases/tag/v1.26.0-mainnet |
| 创世配置 | https://github.com/PIJSChain/pijs/releases/download/v1.26.0-mainnet/genesis.json |
| 引导节点 | https://github.com/PIJSChain/pijs/releases/download/v1.26.0-mainnet/bootnodes.txt |
...
| # | 步骤 | 说明 |
|---|------|------|
| 1 | 停止节点 | 自动检测并停止运行中的节点进程 |
| 2 | 备份客户端 | 备份旧的 geth 到 `backup/` 目录 |
| 3 | 下载新客户端 | 从 GitHub release 下载对应平台的二进制 |
| 4 | 下载创世配置 | 更新 `genesis.json`（硬分叉可能需要） |
| 5 | 重新初始化链 | 执行 `geth init` 更新链配置，**保留链数据** |
| **6** | **网络配置（可选）** | **可选择是否重新配置 NAT 模式** |

**步骤 6「网络配置」**是可选步骤，默认跳过。如果您遇到以下情况，请在提示 "是否重新配置网络？(y/N)" 时输入 `y`：

- 节点更换服务器或公网 IP 变更
- 从家用网络迁移到云服务器（或反之）
- 升级前 `net.peerCount` 长期为 0 或远低于预期
- 之前部署时使用的是旧版 setup 脚本（无 NAT 模式选择）

重新配置后会提供三种模式：

1. **固定公网IP** — 服务器有固定公网IP（推荐用于云主机）
2. **NAT环境** — 家用路由器/NAT 后面，使用 UPnP/NAT-PMP
3. **自动检测（推荐）** — 每次启动时自动查询公网IP，适合动态 IP 的云主机

> **注意**：步骤 6 只修改 `start-node.sh` / `start-node.ps1` 的 NAT 配置部分，**不影响其他启动参数**（如提款地址、引导节点、RPC 端口等）。原文件会自动备份到 `backup/` 目录。

#### Linux / macOS

```bash
# 下载升级脚本
curl -LO https://raw.githubusercontent.com/PIJSChain/chain-stake-guidelines/main/mainnet/zh/scripts/upgrade-node.sh
...
```powershell
# 下载升级脚本
Invoke-WebRequest -Uri "https://raw.githubusercontent.com/PIJSChain/chain-stake-guidelines/main/mainnet/zh/scripts/upgrade-node.ps1" -OutFile "upgrade-node.ps1"
...
```bash
# 下载脚本
curl -LO https://raw.githubusercontent.com/PIJSChain/chain-stake-guidelines/main/mainnet/zh/scripts/update-node-binaries.sh
...
```powershell
# 下载脚本
Invoke-WebRequest -Uri "https://raw.githubusercontent.com/PIJSChain/chain-stake-guidelines/main/mainnet/zh/scripts/update-node-binaries.ps1" -OutFile "update-node-binaries.ps1"
...
3. **下载新客户端**
   ...
   从以下地址下载对应平台的版本：
   <https://github.com/PIJSChain/pijs/releases/tag/v1.26.0-mainnet>

   ```bash
   # 以 Linux x86_64 为例
   cd ~/pijs-node/bin
   curl -LO https://github.com/PIJSChain/pijs/releases/download/v1.26.0-mainnet/geth-linux-amd64
...
4. **下载新的创世配置**
   ...
   ```bash
   cd ~/pijs-node
   curl -LO https://github.com/PIJSChain/pijs/releases/download/v1.26.0-mainnet/genesis.json
   ```
...
6. **（可选）重新配置网络**
   ...
   仅在服务器 IP 变更、网络环境改变、或升级前 peer 数长期为 0 时执行。编辑 `start-node.sh`（或 `start-node.ps1`）中 NAT 配置部分，根据部署环境选择：

   - **固定公网IP**：`NAT_CONFIG="extip:<你的公网IP>"`
   - **NAT环境**（家用）：`NAT_CONFIG="any"`
   - **自动检测**（动态IP）：启动时调用脚本查询公网IP后写入 `NAT_CONFIG`

   > 推荐使用升级脚本的步骤 6 自动完成此项，避免手动编辑出错。
...
```bash
# 下载一键部署脚本
curl -LO https://raw.githubusercontent.com/PIJSChain/chain-stake-guidelines/main/mainnet/zh/scripts/setup-node-testnet.sh
...
```powershell
# 下载一键部署脚本
Invoke-WebRequest -Uri "https://raw.githubusercontent.com/PIJSChain/chain-stake-guidelines/main/mainnet/zh/scripts/setup-node-testnet.ps1" -OutFile "setup-node-testnet.ps1"
...
**下载地址**: <https://github.com/PIJSChain/pijs/releases/tag/v1.26.0-mainnet>

| 平台 | 文件名 |
| ---- | ------ |
| Linux x86_64 | geth-v1.26.0-mainnet-linux-amd64.tar.gz |
| Linux ARM64 | geth-v1.26.0-mainnet-linux-arm64.tar.gz |
| macOS Intel | geth-v1.26.0-mainnet-darwin-amd64.tar.gz |
| macOS Apple Silicon | geth-v1.26.0-mainnet-darwin-arm64.tar.gz |
| Windows x86_64 | geth-v1.26.0-mainnet-windows-amd64.tar.gz |
...
```bash
# 下载
wget https://github.com/PIJSChain/pijs/releases/download/v1.26.0-mainnet/geth-v1.26.0-mainnet-linux-amd64.tar.gz

# 解压
tar -xzf geth-v1.26.0-mainnet-linux-amd64.tar.gz
...
```bash
wget https://github.com/PIJSChain/pijs/releases/download/v1.26.0-mainnet/geth-v1.26.0-mainnet-linux-arm64.tar.gz
tar -xzf geth-v1.26.0-mainnet-linux-arm64.tar.gz
sudo mv geth bootnode /usr/local/bin/
geth version
```
...
```bash
curl -LO https://github.com/PIJSChain/pijs/releases/download/v1.26.0-mainnet/geth-v1.26.0-mainnet-darwin-amd64.tar.gz
tar -xzf geth-v1.26.0-mainnet-darwin-amd64.tar.gz
sudo mv geth bootnode /usr/local/bin/
geth version
```
...
```bash
curl -LO https://github.com/PIJSChain/pijs/releases/download/v1.26.0-mainnet/geth-v1.26.0-mainnet-darwin-arm64.tar.gz
tar -xzf geth-v1.26.0-mainnet-darwin-arm64.tar.gz
sudo mv geth bootnode /usr/local/bin/
geth version
```
...
1. 下载 `geth-v1.26.0-mainnet-windows-amd64.tar.gz`
...
# Windows PowerShell
Invoke-WebRequest -Uri "https://github.com/PIJSChain/pijs/releases/download/v1.26.0-mainnet/genesis.json" -OutFile "genesis.json"
```
...
质押到期后可在 **72 小时**内解锁。72 小时内未解锁将自动进入下个周期。测试网质押期限为 7 天，主网默认质押期限为 180 天。
