...
|----------|--------------|
| Node Binary | https://github.com/PIJSChain/pijs/releases/tag/v1.26.0-mainnet |
| Genesis Config | https://github.com/PIJSChain/pijs/releases/download/v1.26.0-mainnet/genesis.json |
| Bootstrap Nodes | https://github.com/PIJSChain/pijs/releases/download/v1.26.0-mainnet/bootnodes.txt |

---

## Node Upgrade (For Existing Nodes)

> If you are deploying a new node, skip this section and go to [Prerequisites](#prerequisites)

When a new version is released (especially for hard fork upgrades), existing nodes need to upgrade client and re-initialize chain config.
...
curl -LO https://raw.githubusercontent.com/PIJSChain/chain-stake-guidelines/main/mainnet/en/scripts/upgrade-node.sh
...
```powershell
# Download upgrade script
Invoke-WebRequest -Uri "https://raw.githubusercontent.com/PIJSChain/chain-stake-guidelines/main/mainnet/en/scripts/upgrade-node.ps1" -OutFile "upgrade-node.ps1"
...
```bash
# Download script
curl -LO https://raw.githubusercontent.com/PIJSChain/chain-stake-guidelines/main/mainnet/en/scripts/update-node-binaries.sh
...
```powershell
# Download script
Invoke-WebRequest -Uri "https://raw.githubusercontent.com/PIJSChain/chain-stake-guidelines/main/mainnet/en/scripts/update-node-binaries.ps1" -OutFile "update-node-binaries.ps1"
...
5. **Re-initialize chain config (for hard fork)**
   ...
   > **Important**: This only updates chain config, does NOT delete existing chain data
...
After restart, verify the upgrade:

```bash
# Check version
./bin/geth version

# Connect to console and check sync status
...
curl -LO https://raw.githubusercontent.com/PIJSChain/chain-stake-guidelines/main/mainnet/en/scripts/setup-node-testnet.sh
...
```powershell
# Download deployment script
Invoke-WebRequest -Uri "https://raw.githubusercontent.com/PIJSChain/chain-stake-guidelines/main/mainnet/en/scripts/setup-node-testnet.ps1" -OutFile "setup-node-testnet.ps1"
...
**Download URL**: <https://github.com/PIJSChain/pijs/releases/tag/v1.26.0-mainnet>
...
1. Download `geth-v1.26.0-mainnet-windows-amd64.tar.gz`
