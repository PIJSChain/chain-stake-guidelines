...
RELEASES_API="https://api.github.com/repos/${GITHUB_REPO}/releases?per_page=30"
RELEASE_TAG_REGEX='-mainnet$'        # Only match MainNet release tags
...
fetch_latest_version() {
    print_step "1" "Checking the latest MainNet release"

    local response
    response=$(download_text "$RELEASES_API")

    # Extract tag_name list (newest first) and pick the first MainNet tag
    ...
    if [ -z "$LATEST_VERSION" ]; then
        print_error "No MainNet release found (expected tag suffix: -mainnet)"
        exit 1
    fi

    GITHUB_RELEASE="https://github.com/${GITHUB_REPO}/releases/download/${LATEST_VERSION}"
    print_success "Latest MainNet version: $LATEST_VERSION"
}
