...
# ============================================================
# Picks the most recent MainNet release tag (suffix `-mainnet`) from PIJSChain/pijs.
# That repo hosts both TestNet (no/`-testnet` suffix) and MainNet (`-mainnet` suffix)
# releases, so a plain `releases/latest` query is unreliable for MainNet operators.
# ============================================================
...
$ReleasesApi = "https://api.github.com/repos/$GitHubRepo/releases?per_page=30"
$ReleaseTagPattern = "*-mainnet"     # Only match MainNet release tags
...
function Get-LatestVersion {
    Write-Step "2" "Checking the latest MainNet release"

    try {
        $releases = Invoke-RestMethod -Uri $ReleasesApi
    } catch {
        Write-Error-Custom "Failed to query GitHub Releases: $_"
        exit 1
    }

    # GitHub returns releases newest first; pick the first one matching the MainNet tag pattern
    $mainnetRelease = $releases | Where-Object { $_.tag_name -like $ReleaseTagPattern } | Select-Object -First 1

    if (-not $mainnetRelease) {
        Write-Error-Custom "No MainNet release found (expected tag suffix: -mainnet)"
        exit 1
    }

    $script:LatestVersion = $mainnetRelease.tag_name
    $script:ReleaseDownloadBase = "https://github.com/$GitHubRepo/releases/download/$($script:LatestVersion)"
    Write-Success "Latest MainNet version: $($script:LatestVersion)"
}
